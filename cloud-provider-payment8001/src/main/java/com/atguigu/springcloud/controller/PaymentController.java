...
public class PaymentController {
    ...
    public CommenResult savePayment(@RequestBody Payment payment){
        ...
        if (result > 0){
            return new CommenResult(200,"插入成功"+port,result);
        }else{
            return new CommenResult(-1,"插入失败"+port,null);
        }
    }
    ...
    public CommenResult getPaymentById(@PathVariable("id") Integer id) {
        ...
        if (Objects.isNull(result)){
            return new CommenResult(-1,"无ID为{}的数据"+id+"_"+port,null);
        }else {
            return new CommenResult(200,"查询成功"+port,result);
        }
    }
}
